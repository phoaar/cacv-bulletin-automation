...
const { datesMatch, URL_REGEX } = require('./utils');
...
function validateBulletin(data) {
  ...
  if (roster && roster.length > 0) {
    if (roster[0] && !roster[0].preacher) issues.push('Upcoming roster: Preacher is not assigned');
    if (roster[0] && !roster[0].paSound)  issues.push('Upcoming roster: PA / Sound is not assigned');
  }

  // ── Service date vs roster date mismatch ──────────────────────────────────
  if (roster && roster.length > 0 && service.date && roster[0]) {
...
async function validateLinks(announcements) {
  ...
  // Use the global URL_REGEX with the 'g' flag for matching
  const globalUrlRegex = new RegExp(URL_REGEX.source, 'gi');
  const checks = [];
  
  for (const ann of announcements) {
    if (!ann) continue;
    const text = `${ann.title || ''} ${ann.body || ''}`;
    const urls = [...new Set(text.match(globalUrlRegex) || [])];
